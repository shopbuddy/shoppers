...
Twitter.prototype.fetchTimelines = function(elm) {
  var message = {
    ...
    // action should be saved as variable so this can be reused.
  ...
  // https://api.twitter.com/1.1/account/settings.json => "screen_name"
  ...
  // display user name test
  $.ajax({
    "type": "GET",
    "url": OAuth.addToURL(message.action, message.parameters),
    "dataType": "json",
    "success": function(data) {
      var root = $("<div>").attr("class", "screen_name");

      root.append('<p>' + data.screen_name + '</p>');

      elm.removeChild(elm.querySelector("#twitter-login"));
      $(elm).append(root);
      $(elm.querySelector("#selection-dropdown")).css("display", "block");
    },
    "error": function(xhr, status, error) {
      if (xhr.status === 401) {
