...
$("#enter").click(function(){
    ...
    if (product) {
        ...
        if (rec_uid_raw) {
            ...
            uniq = [];
            $.each(rec_uid, function(i, el) { if($.inArray(el, uniq) == -1) uniq.push(el);});
            if (uniq.length) {
                output = "";
      ...
      $("#recom").html("");
            ...
            $(".dim").click(function() {
                ...
                $("#recom").hide(), $("#show").show(), $("footer").show(), y = $(this).attr("alt"), b = $(this).attr("id"),w = "http://graph.facebook.com/{uid}/picture",
                E = w.replace("{uid}", b),$('<img class="ispec"></img>').attr("src", E).appendTo("#show").fadeIn("slow");
...
$("#torec").click(function() {
    "use strict";
    $("#intro").hide(),$("#show").hide(),$("#friend").hide(),$("#show").html(""),$("#recom").show(),$("header").hide(),$("footer").hide();
});
