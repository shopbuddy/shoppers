...
function ImgLoad() {
  "use strict";
    ...
    i = JSON.parse(r), s = i.data.length, o = C(0, s), u = o(), u > s - 40 && (u = s - 40), d = "";
    for (f = u; f < u + 40; f += 1) l = i.data[f].id, c = i.data[f].name, h = e.replace("{uid}", l), p = '<img src="' + h + '" ' + 'id="' + l + '" ' + 'alt="' + c + '" ' + 'class="iam' + '">', d += p;
    $("#friends").append(d), $(".iam").click(function() {
        $("#intro").hide(), $("#friends").toggle(), $("#show").toggle();
    }), $(document).ready(function() {
        $(".iam").hover(function() {
            v = $(this).attr("alt"), $('<p class="tooltip"></p>').text(v).appendTo("body").fadeIn("slow");
        }, function() {
  ...
  $("#show").html(""),$("#torec").show();
        $("#friends").hide(), $("#show").show(), $("footer").show(), y = $(this).attr("alt"), b = $(this).attr("id"), w = "http://graph.facebook.com/{uid}/picture", E = w.replace("{uid}", b), $('<img class="ispec"></img>').attr("src", E).appendTo("#show").fadeIn("slow");

  if (typeof(bg.u) === "undefined") {
         console.log("Product is not selected");
  } else {
     $("header").show(), x = localStorage.getItem("lips");
     if (x) {
         $('<p class="spec"></p>').text("You have selected " + y + " as your Shop Buddy.Go Ahead,send him/her a facebook msg or Share on your facebook wall.").appendTo("#show").fadeIn("slow"),
            ...
            $("header").show(), $(".msg").replaceWith('<a href="' + T + '" ' + 'target="_blank"' + ">" + '<img src="img/msg.png">' + "</a>"), $(".feed").replaceWith('<a href="' + N + '" ' + 'target="_blank"' + ">" + '<img src="img/feed.png">' + "</a>")
...
$("#enter").click(function() {
    "use strict";
    $("#intro").hide(), $("#login").show();
});

var uinfo = localStorage.getItem("user"), me, img, imgurl;

if (uinfo) {

me = JSON.parse(uinfo), $('<p class="loginText"></p>').text("Welcome " + me.name).appendTo("#login"), img = "https://graph.facebook.com/{userid}/picture", imgurl = img.replace("{userid}", me.id), $('<img class="loginImg">').attr("src", imgurl).appendTo("#login"), ImgLoad();
...
$("#choose").click(function() {
    "use strict";
    $("#login").hide(), $("#friends").show(),$("#torec").hide(),$("footer").show();
...
$("#shuffle").click(function() {
    "use strict";
    $("#intro").hide(), $("#show").hide(), $("header").hide(), $("#friends").fadeOut("slow"), $("#friends").html(""), $("#show").html(""), ImgLoad(), $("#friends").fadeIn("slow"), $("#torec").hide();
});
